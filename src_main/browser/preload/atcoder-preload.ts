        ...
        if (ele.innerText?.includes("入力")) {
          if (ele.querySelector("span")) {
            ...
            buttonObject.onclick = function (clickEvent) {
              ...
              const sampleID = ele.innerText?.slice(numindex, endindex).trim();
              ...
              // 初回DOM書き込み
              if (!document.querySelector(`#result-stdout-case-${sampleID}`)) {
              ...
              ipcRenderer.on(
                ...
                (e, info: atcoderCodeTestResult) => {
                  if (
                    info.testGroup === `view-button:${window.location.pathname}`
                  ) {
